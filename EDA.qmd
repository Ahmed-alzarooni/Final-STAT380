...
```{R}
ldc_data <- read_excel("LDC_data.xlsx") %>% 
    rename_with(tolower) %>% 
    drop_na() %>%
...
Create some visualizations to understand the data better. For instance, if you have a numeric variable like 'total_energy_consumption' in the energy dataset, you could plot it.
```{R}
# Histogram of a numeric variable in ldc_data
ggplot(ldc_data, aes(x = numeric_column_name)) + 
  geom_histogram(bins = 30, fill = "blue", color = "black")

# Line plot of energy consumption over time in energy_data
ggplot(energy_data, aes(x = year, y = total_energy_consumption)) + 
  geom_line() + 
  labs(title = "Total Energy Consumption Over Time", x = "Year", y = "Energy Consumption")
